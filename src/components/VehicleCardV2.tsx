...
const VehicleCardV2: Component<VehicleCardV2Props> = (props) => {
    ...
    const getModeLabel = (mode: number) => {
        // 0: Offline/Unknown, 1: Active, ... depending on firmare. 
        // Using the map from user request as a guide for what 'status' might mean in display
        // But strictly, 'mode' from heartbeat might differ from 'status' command. 
        // For now, let's just display the raw mode or map basic ones if known.
        // Existing VehicleCard maps: 0=Offline, 1=Working, 2=Stop Relay1, 3=Stop Relay2, 6=PM
        switch (mode) {
            case 0: return "Offline";
            case 1: return "ทำงาน";
            case 2: return "หยุด รีเลย์ 1 (ยกงา)";
            case 3: return "หยุด รีเลย์ 2 (เคลื่อนที่)";
            case 6: return "PM";
    ...
    return (
        <div id={props.id} class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden flex flex-col h-full transition-all duration-200 hover:shadow-xl">
            {/* Card Header */}
            <div class="px-4 py-3 border-b border-gray-100 flex justify-between items-center bg-white">
                ...
                <span class={`badge px-2 py-0.5 rounded text-xs font-semibold ${status() === "connected"
            ...
            <div class="p-4 flex-1 flex flex-col">
                ...
                <div class="mt-4 mb-3 space-y-1">
                    <p class="text-sm text-gray-600 flex justify-between border-b border-dashed border-gray-200 pb-1">
                        <span>S/N กล่อง:</span>
                        <span class="font-mono text-gray-800">{props.vehicle.box_serial_number || "-"}</span>
                    </p>
                    <p class="text-sm text-gray-600 flex justify-between">
                        <span>S/N รถ:</span>
                        <span class="font-mono text-gray-800">{props.vehicle.serial_number}</span>
                    </p>
                </div>
