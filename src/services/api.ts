...
export const LIFTNGO_URL =
  import.meta.env.VITE_LIFTNGO_URL || "https://uat.liftandgosolutions.com";
...
export const api = {
  ...
  getVehicles: async (
    ...
    const response = await fetch(
      `${BASE_URL}/technician/fleet-boxes?page=${page}&limit=${limit}&search=${search}`,
      {
        headers: getHeaders(),
      },
    ).then(handleResponse);
    ...
    const vehicles = items.map((item: any) => ({
      ...
      model:
        item.product_name ||
        item.fleet_product?.product_name ||
        "Unknown Model",
  ...
  getHistory: async (
    ...
    endDate = "",
    search = "",
  ): Promise<PaginatedResponse<HistoryLog>> => {
  ...
  approveVehicle: async (
    ...
    try {
      ...
      formData.append("matching_process", "4");
      ...
      const externalResponse = await fetch(
        `${LIFTNGO_URL}/api/prove_matching`,
        {
          method: "POST",
          headers: {
  ...
  logout: async (): Promise<void> => {
    try {
      ...
      const tokenForLogout =
        liftngoToken ||
        (currentToken && /^\d+\|/.test(currentToken) ? currentToken : null);
