...
const VehicleHistory = () => {
    ...
    const getDefaultDates = () => {
        ...
        const defaultStart = new Date(now.getTime() - defaultDaysBack * 24 * 60 * 60 * 1000);
    ...
    const [historyData] = createResource(
        () => ({ page: page(), start: startDate(), end: endDate(), search: searchTerm() }),
    ...
    const meta = () => historyData()?.meta || { total: 0, page: 1, limit: 5, totalPages: 1 };
        ...
        <div class="min-h-screen bg-primary transition-colors duration-300">
            ...
            <main class="max-w-6xl mx-auto px-4 py-8">
                <div class="bg-secondary rounded-xl shadow-sm border border-border-primary overflow-hidden">
                                ...
                                {(log) => (
                                    <div class="p-4 hover:bg-tertiary/5 transition-colors group border-b border-border-secondary last:border-0">
                                        <div class="flex items-start gap-4">
                                            ...
                                            <div class={`mt-1 w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${log.action.includes('OPEN') || log.action.includes('ACTIVATE')
                                                ? 'bg-emerald-500/10 text-emerald-500'
                                                : log.action.includes('CLOSE') || log.action.includes('DEACTIVATE')
                                                    ? 'bg-rose-500/10 text-rose-500'
                                                    : 'bg-accent/10 text-accent'
                                                }`}>
                                                <Show when={log.action.includes('OPEN') || log.action.includes('ACTIVATE')} fallback={
                                                    <Show when={log.action.includes('CLOSE') || log.action.includes('DEACTIVATE')} fallback={
                    ...
                    {/* Enhanced Pagination */}
                    <Show when={!historyData.loading && logs().length > 0}>
                        <div id="tour-history-pagination" class="p-4 border-t border-border-secondary bg-tertiary/30">
                            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                                <div class="text-xs text-text-secondary">
                                    Page <span class="font-bold text-text-primary">{meta().page}</span> of <span class="font-bold text-text-primary">{meta().totalPages}</span>
                                </div>
